...
export function registrarServiceWorker() {
  ...
  window.addEventListener('load', async () => {
    try {
      ...
      /*
       * El navegador solo comprueba el service worker cuando hay una navegación
       * de verdad. Aquí el enrutador es por hash, así que moverse entre
       * pantallas no cuenta: con la pestaña abierta no se pregunta nunca y el
       * aviso de versión nueva no llega a aparecer. La persona publica un
       * cambio, recarga —incluso con Ctrl+Shift+R, que se salta la caché pero
       * no echa a un service worker que está sirviendo— y sigue viendo lo
       * viejo hasta que cierra todas las pestañas del sitio.
       *
       * Se pregunta al volver a la pestaña y al recuperar la conexión, que son
       * los dos momentos en que puede haber algo nuevo y la persona está a
       * punto de mirar. Sin `setInterval`: un temporizador permanente gasta
       * batería en un teléfono para responder casi siempre que no hay nada
       * (criterio 6.8).
       */
      let ultimaConsulta = 0;
      const revisarVersion = () => {
        const ahora = Date.now();
        if (ahora - ultimaConsulta < 60_000) return;
        ultimaConsulta = ahora;
        registro.update().catch(() => { /* sin red: ya se reintentará */ });
      };
