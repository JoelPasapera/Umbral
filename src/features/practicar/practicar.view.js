...
function bloqueVeredicto() {
  const { acerto, explicacion, porQueFallaste } = estado.veredicto;
  const cuerpo = el('p', { clase: 'veredicto-practica__cuerpo' });
  escribirConFormulas(cuerpo, explicacion);

  // El análisis del error propio va ANTES de la solución general, y no
  // después: el alumno que acaba de fallar lee la primera línea y a veces no
  // más. Lo que le sirve es entender su razonamiento, no ver el correcto.
  //
  // Cuando no hay explicación para su alternativa, esto no aparece y la
  // pantalla queda exactamente como estaba. Nunca se anuncia lo que falta.
  let propio = null;
  if (!acerto && porQueFallaste) {
    const texto = el('p', { clase: 'veredicto-practica__cuerpo' });
    escribirConFormulas(texto, porQueFallaste);
    propio = el('div', { clase: 'veredicto-practica__tuyo' }, [
      el('p', { clase: 'veredicto-practica__subtitulo', texto: 'Por qué tu alternativa fallaba' }),
      texto,
    ]);
  }
  ...
  return el(
    ...
    [
      ...
      propio,
      propio && el('p', { clase: 'veredicto-practica__subtitulo', texto: 'El camino correcto' }),
      cuerpo,
    ],
  );
}
...
async function enviar() {
  if (estado.seleccion === null || estado.veredicto || estado.comprobando) return;
...
function pintar() {
  ...
  const acciones = estado.veredicto
    ? el('button', {
        ...
        type: 'button',
        texto: estado.indice + 1 >= estado.sesion.total ? 'Ver resultado' : 'Siguiente pregunta',
        on: { click: avanzar },
      })
    : el('button', {
        clase: 'boton boton--ancho',
        type: 'button',
        texto: estado.comprobando ? 'Comprobando' : 'Comprobar',
        attrs: {
          disabled: estado.seleccion === null || estado.comprobando,
          'aria-busy': estado.comprobando ? 'true' : null,
        },
        on: { click: enviar },
      });
...
/** @param {{ curso?: string, modo?: string, examen?: string }} params */
export async function render(params) {
  ...
  // Un examen abierto desde la biblioteca entra en su propio modo: son las
  // preguntas de esa convocatoria y de ninguna otra.
  const modo = params.examen ? 'examen' : params.modo;
  const sesion = await iniciar({ cursoId: params.curso, modo, examenId: params.examen ?? null, debiles });
  ...
  if (!sesion.total) {
    montar(
      estado.raiz,
      estadoVacio(modo === 'examen'
        ? {
          // No se disimula ni se rellena con otras preguntas: se dice cuál es
          // el hueco, que es lo único útil que se puede decir aquí.
          titulo: 'Este examen todavía no está cargado',
          cuerpo: 'El PDF sí está disponible en la biblioteca. Las preguntas para resolverlo dentro de Umbral se cargan una por una y esta convocatoria aún no las tiene.',
          accion: { texto: 'Volver a la biblioteca', href: '#/biblioteca' },
        }
        : {
