...
function tarjeta(material) {
  ...
  // Antes esto era "Resumen · Fuente · 12 min", la cadena con puntos medios que
  // aparece en cualquier plantilla. El tipo ya lo dice el icono; lo que el
  // alumno necesita saber antes de entrar es cuánto le va a costar.
  // Sin duración no se escribe la línea. Un material que no la trae pintaba
  // "undefined min", que no lanza ningún error y se ve en pantalla.
  const meta = material.minutos
    ? [el('span', { clase: 'material__duracion', texto: `${material.minutos} min` })]
    : [];
  if (material.fuente) meta.push(el('span', { clase: 'material__fuente', texto: material.fuente }));
