 ...
 * Entrada con Google.
 *
 * En producción esto redirige al proveedor y vuelve con un código que el
 * servidor canjea; aquí el servidor simulado devuelve el resultado directo.
 * El contrato es el mismo en los dos casos, así que la vista no cambia.
 *
 * @param {{ perfil?: string }} [opciones]
 * @returns {Promise<{usuario?:object, pase?:string, perfil?:object, necesitaCodigo:boolean}>}
 */
export async function entrarConGoogle(opciones = {}) {
  const respuesta = await pedir('auth/google', opciones);
  if (respuesta.token) guardarToken(respuesta.token);
  return respuesta;
}

/**
 * Termina el registro de una identidad de Google. Pide lo mismo que el
 * registro normal: código de academia, año de nacimiento y términos.
 * @param {object} datos
 */
export async function completarConGoogle(datos) {
  const { token, usuario } = await pedir('auth/google/completar', datos);
  guardarToken(token);
  return usuario;
}
