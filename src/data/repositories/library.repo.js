...
export async function resumen(id) {
  return pedir('estudio/resumen', { id, token: localStorage.getItem('umbral:sesion') });
}
...
/**
 * Catálogo con sus facetas: las tres colecciones, los cursos ordenados por
 * peso del examen y cuánto del examen no tiene material.
 *
 * Los recuentos llegan calculados del servidor y no se derivan en la vista:
 * un recuento hecho en el cliente solo puede contar lo que se le mandó, y lo
 * que interesa es lo que existe.
 *
 * @param {{ origen?:string, cursoId?:string, universidadId?:string, busqueda?:string }} filtros
 */
export async function catalogo(filtros = {}) {
  return pedir('biblioteca/catalogo', {
    ...filtros,
    token: localStorage.getItem('umbral:sesion'),
  });
}
