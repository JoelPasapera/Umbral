...
/**
 * Manda el token porque el servidor necesita saber de qué academia es quien
 * responde: la explicación del fallo puede ser propia de su academia, y esa
 * frontera se decide con la sesión, nunca con lo que diga la petición.
 *
 * @param {{ sesionId:string, preguntaId:string, opcion:number, segundos:number }} params
 * @returns {Promise<{acerto:boolean, correcta:number, explicacion:string, porQueFallaste?:string}>}
 */
export async function responder(params) {
  return pedir('practica/responder', { ...params, token: localStorage.getItem('umbral:sesion') });
}
