...
export async function crearMaterial({ token, ...datos }) {
  ...
  // Subir material es hacer una declaración: quién lo hizo y con qué derecho
  // está aquí. Por defecto se declara propio de la academia porque ese es el
  // caso normal —la separata del profesor—, pero la declaración se guarda con
  // el material y `domain/catalogo.js` no muestra nada que no la traiga. Sin
  // esto, un escaneo de editorial entra sin que nadie llegue a decidirlo, que
  // es exactamente como se llenó la biblioteca del sitio que se auditó.
  const origen = String(datos.origen ?? 'academia');
  const licencia = String(datos.licencia ?? 'propia');
  const fuente = limpiar(datos.fuente, 100) || `Elaborado por ${usuario.academia ?? 'la academia'}`;

  if (licencia === 'permiso' && !limpiar(datos.permisoRef, 200)) {
    throw new Error('Si el material es de un tercero con permiso, indica dónde está archivado ese permiso.');
  }
  ...
  const material = {
    ...
    licencia,
    fuente,
    permisoRef: limpiar(datos.permisoRef, 200) || null,
    universidadId: limpiar(datos.universidadId, 40) || null,
    cursoId: datos.cursoId,
    temaId: limpiar(datos.temaId, 60),
