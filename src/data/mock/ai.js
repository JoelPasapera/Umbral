...
/**
 * Descuenta créditos del presupuesto de la academia, o lanza si no alcanzan.
 *
 * El presupuesto es uno solo para todo lo que le cuesta dinero a la academia,
 * no uno por funcionalidad: lo que le importa a la coordinación es la factura
 * del mes, no en qué se repartió. Por eso esta función vive aquí y la usan
 * tanto la generación de preguntas como las explicaciones de fallo.
 *
 * @param {{ academiaId:string, creditos:number, concepto?:string }} params
 * @returns {{ usado:number, tope:number, restante:number }}
 */
export function cobrar({ academiaId, creditos, concepto = 'esta operación' }) {
  const p = presupuestoDe(academiaId);
  if (p.usado + creditos > p.tope) {
    const quedan = Math.max(0, p.tope - p.usado);
    throw new Error(
      `Faltan créditos para ${concepto}: hacen falta ${creditos} y quedan ${quedan} este mes. Se renueva el día 1.`,
    );
  }
  presupuestos.set(academiaId, { ...p, usado: p.usado + creditos });
  return estadoPresupuesto({ academiaId });
}
