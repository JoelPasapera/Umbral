...
// El examen reparte preguntas, no porcentajes: todos los cursos entran, pero
// con pesos muy distintos. Trigonometría vale 6 preguntas en ingenierías y 1
// en humanidades, y esa diferencia es el producto.
const trigSistemas = sistemas.cursos.find((c) => c.cursoId === 'trigonometria').preguntas;
const trigDerecho = derecho.cursos.find((c) => c.cursoId === 'trigonometria').preguntas;
ok('el peso en preguntas cambia mucho entre áreas', trigSistemas >= trigDerecho * 3,
  `→ trigonometría: ${trigSistemas} preguntas en Sistemas, ${trigDerecho} en Derecho`);

ok('los pesos suman cien por ciento',
  [sistemas, derecho].every((e) => Math.abs(e.cursos.reduce((s, c) => s + c.peso, 0) - 100) < 0.01));

ok('cada curso declara sus preguntas y sus puntos',
  sistemas.cursos.every((c) => c.preguntas > 0 && c.puntos === c.preguntas * 20));

ok('el examen declara si sus pesos están verificados',
  derecho.pesosVerificados === false && typeof sistemas.pesosVerificados === 'boolean');
